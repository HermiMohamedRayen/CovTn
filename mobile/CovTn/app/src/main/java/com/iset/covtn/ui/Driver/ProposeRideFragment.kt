...
import com.iset.covtn.databinding.FragmentProposeRideBinding
import com.iset.covtn.databinding.FragmentSearchRideBinding
import com.iset.covtn.ui.viewModel.UserViewModel

class ProposeRideFragment : Fragment(), OnMapReadyCallback {

    private var _binding: FragmentProposeRideBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentProposeRideBinding.inflate(inflater, container, false)
        ...
        userViewModel.user.observe(viewLifecycleOwner){
            value -> if(value.profilePicture.isNullOrEmpty() || value.number.isNullOrEmpty() ){
                Toast.makeText(requireContext(),"please add your phone number and profile photo",Toast.LENGTH_LONG).show()
                findNavController().navigate(R.id.nav_home)

                return@observe
        }else {
            if (value.car == null) {
                findNavController().navigate(R.id.nav_add_car, null,
                    navOptions {
                        popUpTo (R.id.propose_ride) { inclusive = true }
                    }
                )
        ...
        if (dep == null) {
            ...
            btn.setOnClickListener {
                if (dest == null) {
                    ...
                    select = 2
                    btn.text = "Go"
                    btn.icon = AppCompatResources.getDrawable(requireContext(),R.drawable.outline_add_24)
    ...
    fun submit() {
        ...
        userViewModel.addRide(dep?.position!!,dest?.position!!,depTime,destTime)
        findNavController().navigate(R.id.nav_home)
        select = 0
